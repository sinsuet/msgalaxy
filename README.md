...
> `2026-03-10` 起，项目已冻结下一阶段的 docs-first 重构蓝图；详见：
...
> - `docs/reports/R40_satellite_reference_baseline_20260310.md`
> - `docs/reports/R41_catalog_shell_geometry_upgrade_20260310.md`
> - `docs/reports/R42_comsol_canonical_satellite_physics_20260310.md`
> - `docs/reports/R43_operator_dsl_v4_and_rule_governance_20260310.md`
> - `docs/reports/R44_iteration_review_package_20260310.md`
...
## 1.1 下一阶段架构升级（2026-03-10 已文档冻结）

下一阶段不是在当前“通用盒体布局”上继续堆补丁，而是按以下五个方向重构：

- `SatelliteArchetype`：初始建模从公开卫星原型族出发，先保证“看起来像卫星”；
- `Catalog + Shell + Aperture`：目录件、机壳、面板、开窗/开孔进入统一几何合同；
- `Canonical COMSOL Physics`：真实卫星热/结构/电学路径和诊断级简化路径显式分层；
- `Operator DSL v4 + Rule Engine`：算子改为卫星领域语义，规则分成硬规则与软偏好；
- `IterationReviewPackage`：`vop_maas` 每个接受 step 都输出三场 + 指标 + 前后对比的审阅包。

当前仓内代码**尚未**完成这五项重构的主线替换；但 ADR-0010 ~ 0014 已完成最小共享桥接，其中 DSL v4 family 映射、`IterationReviewPackage` 索引链路与 Blender review sidecar 的 consumer thin-slice 已落地，完整新架构仍需后续分阶段推进。

`2026-03-11` 状态补充：默认真实 COMSOL 垂直 smoke 仍以 `diagnostic_simplified` 物理合同作为已验证稳定链路；`tools/comsol_field_demo/tool_real_comsol_vertical_smoke.py --enable-canonical-thermal-path` 现提供显式 canonical thermal probe，但真实单例 vertical smoke 仍未收敛，不能对外宣称 canonical thermal 已完成验证。详见 `docs/reports/R51_real_comsol_vertical_smoke_20260311.md` 与 `docs/reports/R56_canonical_thermal_probe_20260311.md`。
...
- `iteration_review_package` 最小执行链已落地：
  - 已有 `teacher_demo / research_fast` 双 profile、registry、step/root/profile index
  - 已接入 `summary.json / report.md / visualizations/visualization_summary.txt` 摘要回写
  - 已支持旧 case 最小 adapter（`design_state.json` / `field_exports` / render/summary metadata）
  - `teacher_demo` 已启用 strict field-case gate；`research_fast` 保持轻量链路
- Blender review sidecar / `IterationReviewPackage` 薄切片已具备：可从 run 目录生成 `package_index.json`、`render_bundle.json`、`review_payload.json`、`render_manifest.json v2`、三态 scene script、Blender 场景脚本与 Codex brief，并把 operator family audit 与 semantic caption 摘要写入下游消费面。
- Blender 可视化当前批准的下一阶段目标仍包括离线 companion dashboard、更高保真 teacher/demo 批量渲染与数据集级审阅联查；这些仍是 planned target，不属于当前已实现能力。
...
- Blender Review Package 的离线 `review_dashboard.html`、全量 teacher-demo 高保真批处理与更完整的数据集交互仍未实现；当前已实现的是 `review_payload.json`、`render_manifest.json v2`、三态 scene script、scene audit 与 step/package 索引薄切片。
