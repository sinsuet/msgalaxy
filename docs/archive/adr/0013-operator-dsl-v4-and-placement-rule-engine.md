...
> 2026-03-11 状态更新：
> 本 ADR 仍有效于 DSL v4 动作集合与规则治理边界，但其中 review-package / Blender sidecar 相关消费面表述仅代表历史切片。
> 当前 tracked 主线以 `HANDOFF.md` 与 `0015-same-repo-single-core-scenario-runtime` 为准。
...
- `core.visualization._operator_action_family(...)` 仍是当前 tracked 主线中的 v4 family consumer，`geometry/aperture/thermal/structural/power/mission` 六大家族在现有 visualization 摘要路径中不再大面积落到 `other`；
