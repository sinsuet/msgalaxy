...
from ..protocol import AgentTask, StructuralMetrics, StructuralProposal
...
class StructuralAgent:
    ...
    def _load_system_prompt(self) -> str:
        return (
            "You are the Structural Agent for spacecraft layout optimization.\n"
            "Return a valid StructuralProposal JSON object only.\n"
            "Use only component IDs provided in the prompt.\n"
            "Focus on safety factor, displacement, and modal response."
        )
    ...
    def generate_proposal(
        ...
        try:
            ...
            raise LLMError(f"Structural Agent failed: {exc}") from exc
    ...
    def _build_prompt(
        ...
        prompt = f"""# Structural Optimization Task
...
- max_stress_mpa: {current_metrics.max_stress:.1f}
- max_displacement_mm: {current_metrics.max_displacement:.3f}
- first_modal_freq_hz: {current_metrics.first_modal_freq:.1f}
- safety_factor: {current_metrics.safety_factor:.2f}

## Component Mass Snapshot
"""
        for comp in current_state.components[:5]:
            prompt += f"- {comp.id}: mass={comp.mass:.2f}kg\n"
    ...
    def validate_proposal(
        ...
        if proposal.predicted_metrics.safety_factor < 1.5:
            warnings.append(
                f"low predicted safety_factor ({proposal.predicted_metrics.safety_factor:.2f})"
            )
        if proposal.predicted_metrics.first_modal_freq < 40:
            warnings.append(
                f"low predicted first_modal_freq ({proposal.predicted_metrics.first_modal_freq:.1f}Hz)"
            )
