...
from optimization.operator_program import OperatorAction, OperatorProgram, validate_operator_program
...
_SUPPORTED_ACTIONS: Tuple[str, ...] = ("group_move", "cg_recenter", "hot_spread", "swap")
...
class OperatorProgramGenomeCodec:
    ...
    def __init__(
        ...
        *,
        ...
        self.n_action_slots = max(1, min(int(n_action_slots), 8))
        self._slot_width = 6
    ...
    def encode(self, state: DesignState) -> np.ndarray:
        ...
        vector[offset + 0] = 1.0  # cg_recenter selector
        ...
        if ranked.size >= 1 and self.n_action_slots >= 2:
            ...
            slot = 1
            offset = slot * self._slot_width
            vector[offset + 0] = 2.0  # group_move selector
        ...
        if ranked.size >= 2 and self.n_action_slots >= 3:
            ...
            slot = 2
            offset = slot * self._slot_width
            vector[offset + 0] = 2.8  # hot_spread selector
    ...
    def _build_neutral_genome(self) -> np.ndarray:
        ...
        for slot in range(self.n_action_slots):
            offset = slot * self._slot_width
            neutral[offset + 0] = 1.0  # cg_recenter
    ...
    def _select_action_for_slot(self, *, slot: int, selector: float) -> str:
        if slot <= 0:
            return "cg_recenter"
        normalized = float(np.clip(selector / max(float(len(_SUPPORTED_ACTIONS) - 1), 1.0), 0.0, 1.0))
        if normalized < 0.55:
            return "cg_recenter"
        if normalized < 0.80:
            return "group_move"
