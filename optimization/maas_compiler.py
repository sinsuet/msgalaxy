...
def compile_intent_to_problem_spec(
    ...
    component_aliases = _build_component_aliases(component_order)
    ...
    for obj in intent.objectives:
        objective_specs.append(
            ObjectiveSpec(
                name=obj.name,
                metric_key=_normalize_metric_key(obj.metric_key),
                sense=obj.direction,
    ...
    for cons in intent.hard_constraints:
        constraint_specs.append(
            ConstraintSpec(
                name=cons.name,
                metric_key=_normalize_metric_key(cons.metric_key),
...
def _build_component_aliases(component_order: List[str]) -> Dict[str, str]:
    alias_map: Dict[str, str] = {}
    for idx, component_id in enumerate(component_order, start=1):
        tokens = {
